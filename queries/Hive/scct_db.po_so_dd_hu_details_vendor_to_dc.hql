...
-- ----------- ------- 		----------  ------------ 	--------------------
-- 01.00.00.01 27DEC19		SCCTP4-2996	ssoma			Component Inflow Initial Version
...
from (select * from scct_db.po_master where purch_doc_type_cd='ZUB' AND (del_ind<>'L' OR (del_ind='L' AND gr_qty>0)) and vend_acct_id<>'N/A' and loc_nbr<>'N/A') po 
inner join (select * from scct_db.location_master where loc_typ_cd='01' and org_num<>'1117') a ON po.loc_nbr=a.loc_nbr 
inner join (select * from scct_db.location_master where loc_typ_cd='05' and org_num<>'1117') b ON po.vend_acct_id=b.loc_nbr;

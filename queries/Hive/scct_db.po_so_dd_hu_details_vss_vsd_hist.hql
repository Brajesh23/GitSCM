---jobname:scct-ufposo-hive-stg1-po_so_dd_hu_details_vss_vsd
--------------------------------------------------------------------------
...
-- ----------- ------- 		----------  ------------ 	--------------------
-- 01.00.00.01 06JAN20		SCCTP4-2924	ssoma			Initial Version
...
from (select * from scct_db.po_master WHERE  purch_doc_type_cd='ZBPO' AND (del_ind<>'L' OR (del_ind='L' AND gr_qty>0))) po   
left join (select * from scct_db.so_master where cust_prch_nbr<>'N/A' )so 
...
left join (select * from scct_db.delivery_master where del_ind<>'D' and del_type not in('EL','ELST') and itm_nbr<>'0' ) dm 
on so.sls_ord_nbr=dm.ref_doc_nbr and so.sls_ord_itm_nbr= dm.ref_doc_itm_nbr and po.itm_nbr=dm.itm_nbr
left join (select * from scct_db.handling_master where itm_nbr <> '0') hm 
on dm.delv_doc_itm_nbr=hm.delv_doc_itm_nbr and dm.delv_doc_nbr=hm.delv_doc_nbr and dm.itm_nbr=hm.itm_nbr 
left join (select * from scct_db.item_unit_measure_master where trim(itm_nbr)<>'') ium on dm.itm_nbr=ium.itm_nbr and hm.pckd_uom=ium.alt_uom ;

---jobname:scct-ufposo-hive-stg1-po_so_dd_hu_kpi_details_posodd
--------------------------------------------------------------------------
...
left join scct_db.so_line_kpi so 
on dtl.business_process=so.business_process and dtl.to_node=so.to_node and dtl.po_doc_typ=so.po_doc_typ and
dtl.so_sls_ord_nbr=so.sls_ord_nbr and dtl.so_sls_ord_itm_nbr=so.sls_ord_itm_nbr and dtl.so_itm_nbr=so.itm_nbr
...
WHERE ((dtl.business_process='Stock_Transfer_Intra_Company' and dtl.to_node='DC_VSD' AND dtl.po_doc_typ='ZBPO')
OR (dtl.business_process='Stock_Transfer_Inter_Company' and dtl.to_node='DC_Mast' AND dtl.po_doc_typ='ZATP'));

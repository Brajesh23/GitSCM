---jobname:scct-ufposo-hive-stg1-po_so_dd_hu_details_gpom
--------------------------------------------------------------------------
...
-- 01.00.00.03 06MAR20		SCCTP4-3316	ssoma			remove reducer parameter
-- 01.00.00.04 04MAY20		SCCTP4-3331	ssoma			HQL Optimization 
----------------------------------------------------------------------------

-------------------------

-------------------------
---loading po line kpis data into po_so_dd_hu_details table for GPOM model
-------------------------
...
from (select * from scct_db.po_master where purch_doc_type_cd='Z1PO' and (del_ind<>'L' OR (del_ind='L' AND gr_qty>0)) and loc_nbr<>'N/A' and vend_acct_id<>'N/A')po 
inner join (select * from scct_db.location_master where loc_typ_cd in('01','02','13')) l on po.loc_nbr=l.loc_nbr 
inner join (select * from scct_db.location_master where loc_typ_cd in('99')) v on po.vend_acct_id=v.loc_nbr 
left outer join (select * from scct_db.delivery_master where del_ind<>'D' and itm_nbr<>'0') dm 
on po.purch_ord_nbr=dm.ref_doc_nbr and po.purch_ord_itm_nbr= dm.ref_doc_itm_nbr and po.itm_nbr=dm.itm_nbr 
left outer join (select * from scct_db.so_master where itm_nbr<>'N/A' ) so 
on po.sls_ord_nbr=so.sls_ord_nbr and po.sls_ord_itm_nbr=so.sls_ord_itm_nbr and po.itm_nbr=so.itm_nbr
left outer join (select * from scct_db.handling_master where delv_doc_itm_nbr <> '0' and itm_nbr<>'0') hm 

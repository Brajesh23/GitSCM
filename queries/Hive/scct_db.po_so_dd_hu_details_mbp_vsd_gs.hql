...
---source: tms_stage0 table : po_master,location_master
---load type: full
---back posting: no
--
-- History Information
...
-- ----------- ------- 		----------  ------------ 	--------------------
-- 01.00.00.01 20AUG19		SCCTP4-2931	ssoma			Initial Version
...
,'Vendor_Managed_Mast' as from_node
,po.purch_ord_nbr
,po.purch_ord_itm_nbr
,coalesce(po.del_ind,dm.del_ind,'N/A') as del_ind
,coalesce(po.itm_nbr,dm.itm_nbr,'0') as itm_nbr
...
from (select * from scct_db.po_master where purch_doc_type_cd='UB' AND (del_ind<>'L' OR (del_ind='L' AND gr_qty>0)) 
and vend_acct_id<>'N/A' and loc_nbr<>'N/A' AND itm_nbr like '00000000002%') po 
inner join (select * from scct_db.location_master where loc_typ_cd='05' and org_num='1117') a ON po.vend_acct_id=a.loc_nbr 
inner join (select * from scct_db.location_master where org_num='1117' and ((loc_typ_cd ='02' and loc_nbr in ('D044','D087','D096')) OR loc_typ_cd ='05')) b ON po.loc_nbr=b.loc_nbr
left outer join (select * from scct_db.delivery_master where del_ind<>'D' and del_type not in('EL','ELST') and itm_nbr<>'0') dm on po.purch_ord_nbr=dm.ref_doc_nbr 
and po.purch_ord_itm_nbr= dm.ref_doc_itm_nbr and po.itm_nbr=dm.itm_nbr
group by 
po.purch_ord_nbr
,po.purch_ord_itm_nbr
,coalesce(po.del_ind,dm.del_ind,'N/A')
,coalesce(po.itm_nbr,dm.itm_nbr,'0')
,coalesce(po.co_cd,'N/A')
,coalesce(po.loc_nbr,'N/A')
,coalesce(po.purch_ord_qty,0.000)
,coalesce(po.purch_ord_qty_uom,'N/A')
...
,coalesce(po.po_create_dt,'9999-12-31')
,coalesce(po.ship_to_loc_nbr,'N/A')
,coalesce(po.rec_crtd_dt,'9999-12-31')
,coalesce(po.floorset_dt,'9999-12-31')
,coalesce(case when trim(po.vend_acct_id)='' then null else trim(po.vend_acct_id) end,'N/A')
,coalesce(po.loc_nbr,'N/A')
,po.purch_doc_type_cd
;

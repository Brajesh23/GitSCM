...
-- ----------- ------- 		----------  ------------ 	--------------------
-- 01.00.00.01 20Aug20		SCCTP5-327    	fyuan			ZDTD Initial Version
----------------------------------------------------------------------------
-------------------------
...
from (select * from scct_db.po_master where purch_doc_type_cd='ZDTD' AND (del_ind<>'L' OR (del_ind='L' AND gr_qty>0)) and vend_acct_id<>'N/A' and loc_nbr<>'N/A') po 
inner join (select * from scct_db.location_master where loc_typ_cd IN ('01','05')) a on po.vend_acct_id=a.loc_nbr 
inner join (select * from scct_db.location_master where loc_typ_cd='01') b on po.loc_nbr=b.loc_nbr 
left outer join (select * from scct_db.delivery_master where del_ind<>'D' and del_type in ('NLCC') and itm_nbr<>'0') dm 
